...
function solve2(): number {
    ...
    for (let x in tools.range(xmin, xmax))
        for (let y in tools.range(ymin, ymax))
            for (let z in tools.range(zmin, zmax)) {
                let candidate = JSON.stringify([parseInt(x), parseInt(y), parseInt(z)])
                if (!water.has(candidate)) lava.push(candidate)
            }

    lava.forEach(function (l) {
        let neighbors = getNeighbors(JSON.parse(l))
        neighbors.forEach(function (neighbor) {
            if (!lava.includes(neighbor)) sum++
        })
    })
